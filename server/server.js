...
// Lazy pool initialization - only connect when first query is made
let pool = null;
const getPool = () => {
  if (!pool) {
    pool = new Pool({
...
const initDb = async () => {
  try {
    ...
    const adminCheck = await getPool().query("SELECT count(*) as count FROM admin");
    if (parseInt(adminCheck.rows[0].count) === 0) {
      const hashedPassword = await bcrypt.hash('4141', 10);
      await getPool().query("INSERT INTO admin (password) VALUES ($1)", [hashedPassword]);
...
app.put('/api/activists/:id', async (req, res) => {
  ...
  try {
    const checkResult = await getPool().query(checkSql, [id]);
    ...
    await getPool().query(updateSql, params);
    res.json({
      "message": "success",
      "data": {
        id: parseInt(id),
...
app.delete('/api/activists/:id', async (req, res) => {
  ...
  try {
    ...
    await getPool().query(deleteSql, [id]);
    res.json({
      "message": "success"
    });
  } catch (err) {
...
app.post('/api/admin/login', async (req, res) => {
  ...
  try {
    const result = await getPool().query("SELECT password FROM admin LIMIT 1");
    const row = result.rows[0];
...
app.post('/api/admin/change-password', async (req, res) => {
  ...
  try {
    ...
    await getPool().query("UPDATE admin SET password = $1", [hashedNewPassword]);
    res.json({ "message": "success" });
  } catch (err) {
    res.status(500).json({ "error": err.message });
  }
...
app.delete('/api/admin/activists/:id', async (req, res) => {
  ...
  try {
    ...
    const adminResult = await getPool().query("SELECT password FROM admin LIMIT 1");
    ...
    // Proceed to delete
    const deleteSql = 'DELETE FROM activists WHERE id = $1';
    const deleteResult = await getPool().query(deleteSql, [id]);
